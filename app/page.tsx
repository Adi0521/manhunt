...
export default function HomePage() {
  ...
  const [challengeTimerAnchor, setChallengeTimerAnchor] = useState<Date | null>(null);
  ...
  useEffect(() => {
    const hunt = hunts[hunts.length - 1];
    if (!hunt?.created_at || challengeTimerAnchor !== null) return;
    const threeMinMark = new Date(new Date(hunt.created_at).getTime() + 3 * 60 * 1000);
    if ((huntTime ?? 0) >= 60 * 3) {
      setChallengeTimerAnchor(threeMinMark);
    }
  }, [huntTime, hunts, challengeTimerAnchor]);

  useEffect(() => {
    const hunt = hunts.length > 0 ? hunts[hunts.length - 1] : null;
    const timeoutMins = (hunt as any)?.challenge_timeout_minutes;
    if (!challengeTimerAnchor || !timeoutMins) {
      setChallengeSecondsLeft(null);
      return;
    }
    const deadline = new Date(challengeTimerAnchor.getTime() + timeoutMins * 60 * 1000);
    const tick = () => setChallengeSecondsLeft(Math.max(0, Math.floor((deadline.getTime() - Date.now()) / 1000)));
    tick();
    const interval = setInterval(tick, 1000);
    return () => clearInterval(interval);
  }, [challengeTimerAnchor, hunts]);
  ...
  function completeChallenge() {
    ...
    setChallengeTimerAnchor(new Date());
